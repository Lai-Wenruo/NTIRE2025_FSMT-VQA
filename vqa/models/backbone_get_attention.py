...
class SwinTransformer3D(nn.Module):
    ...
    def inflate_weights(self, logger):
        ...
        axis. To utilize the pretrained parameters in 2d vqa,
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(self.pretrained, str):
            ...
            logger.info(f"load vqa from: {self.pretrained}")

            if self.pretrained2d:
                # Inflate 2D vqa into 3D vqa.
                self.inflate_weights(logger)
            else:
                # Directly load 3D vqa.
    ...
    def train(self, mode=True):
        """Convert the vqa into training mode while keep layers freezed."""
