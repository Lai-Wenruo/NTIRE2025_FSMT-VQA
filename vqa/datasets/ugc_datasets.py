...
class UGCDataset(torch.utils.data.Dataset):
        ...
        if isinstance(self.ann_file, list):
            ...
            try:
                with open(self.ann_file, "r") as fin:
                    for line in fin:
                        line_split = line.strip().split(",")
                        # 针对 "2784.mp4,39.6364,49.2386,46.2638,51.3306,42.5309,46.224" 这种格式做处理
                        if len(line_split) == 7:
                            filename = line_split[0]
                            # 将后面的 6 个数字转为浮点数列表
                            label = list(map(float, line_split[1:]))
    ...
    def __getitem__(self, index):
        ...
        data["gt_label"] = label  # 若 label 为列表，后续需要注意使用方式
...
if __name__ == "__main__":
    ...
    #
    ugc_opt = {
        "anno_file": "./examplar_data_labels/CVPR_UGC/CVPR_UGC_train1.txt",
    ...
    dataset_3 = UGCDataset(ugc_opt)
    print([(key, dataset_3[0][key].shape) for key in ugc_opt["sample_types"]])
