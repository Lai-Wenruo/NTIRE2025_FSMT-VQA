...
def train_test_split(dataset_path, ann_file, ratio=0.6, seed=42):
    random.seed(seed)
    video_infos = []
...
def inference_set(inf_loader, model, device, best_, save_model=False, suffix='s', save_name="divide"):
    ...
    # print("val:pr_labels bef_recale")
    # print(pr_labels)
    # print("val:pr_labels after_recale")
    ...
    if s + p > best_s + best_p and save_model:
        ...
        torch.save(
            ...
            f"pretrained_weights/{save_name}_{suffix}_25_3_13.pth",
        )
...
def main():
    ...
    parser.add_argument(
        "-o", "--opt", type=str, default="./options/train/train-color.yml", help="the option file"
    )
        ...
        if opt["l_num_epochs"] >= 0:
            for key in val_loaders:
                print(
                    ...
                    the best validation accuracy of the vqa-s is as follows:
                ...
                print(
                    f"""For the linear transfer process on {key} with {len(val_loaders[key])} videos,
                    the best validation accuracy of the vqa-n is as follows:
